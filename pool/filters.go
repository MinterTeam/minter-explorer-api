...
func (f SelectByCoinsFilter) Filter(q *orm.Query) (*orm.Query, error) {
	if id, err := strconv.Atoi(f.Coin0); err == nil {
		...
		q = q.Where(`"first_coin"."symbol" = ?`, symbol).Where(`"first_coin"."version" = ?`, version)
	}
	...
	if id, err := strconv.Atoi(f.Coin1); err == nil {
		...
		q = q.Where(`"second_coin"."symbol" = ?`, symbol).Where(`"second_coin"."version" = ?`, version)
	}
